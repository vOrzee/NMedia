...
class PostFragment : Fragment() {
    ...
    private val adapter = CommentAdapter(object : OnInteractionListenerComment {
    ...
    override fun onCreateView(
        ...
        viewModel.dataPost.observe(viewLifecycleOwner) {
            fillBinding(binding,it)
            listeners(binding,it)
            viewModel.getCommentsById(it)
        }
    ...
    private fun fillBinding(binding: FragmentPostBinding, post: Post) {
        with(binding.singlePost) {
            title.text = post.author
            datePublished.text = if(post.published == "") "" else
